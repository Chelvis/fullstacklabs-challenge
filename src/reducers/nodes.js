import { CHECK_NODE_STATUS_FAILURE, CHECK_NODE_STATUS_START, CHECK_NODE_STATUS_SUCCESS, GET_NODE_BLOCK_FAILURE, GET_NODE_BLOCK_SUCCESS } from '../constants/actionTypes';
...
export default function nodesReducer(state = initialState().nodes, action) {
  ...
  switch (action.type) {
    ...
    case GET_NODE_BLOCK_SUCCESS:
      ...
      nodeIndex = state.list.findIndex(p => p.url === action.node.url);
      if (nodeIndex >= 0) {
        list = state.list.map((node, index) => index === nodeIndex ? { ...node, data: action.res.data } : node);
      }
      return {
        ...state,
        list
    ...
    case GET_NODE_BLOCK_FAILURE:
      ...
      nodeIndex = state.list.findIndex(p => p.url === action.node.url);
      if (nodeIndex >= 0) {
        list = state.list.map((node, index) => index === nodeIndex ? { ...node, data: [] } : node);
      }
      return {
        ...state,
        list
